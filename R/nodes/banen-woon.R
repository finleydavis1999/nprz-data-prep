# banen-woon: CBS baangegevens woonadres 2007-2017, PC4 + gemeente scales.
# Source: raw-data/nodes-2018.sqlite, tables woonwerk_woon_19992018_{pc,gem}.
# Output: static/data/parquet/banen-woon-{pc4,gem}.parquet
source("R/lib/parquet.R")

build_banen_woon <- function() {
  ...
  write_parquet_from_query(con, "
    SELECT
      ...
      value AS count,
      value AS weight
    FROM src.woonwerk_woon_19992018_pc
    ORDER BY year, age, opl, inks, sectorcat, soortbaan
  ", "static/data/parquet/banen-woon-pc4.parquet")
  ...
  write_parquet_from_query(con, "
    SELECT
      ...
      value AS count,
      value AS weight
    FROM src.woonwerk_woon_19992018_gem
    ORDER BY year, age, opl, inks, sectorcat, soortbaan
  ", "static/data/parquet/banen-woon-gem.parquet")

  list(
    name        = "Baangegevens Woonadres",
    description = "CBS microdata 2007-2017, banen geteld op woonadres. Cellen <10 onderdrukt voor privacy.",
    scales = list(
      pc4 = "parquet/banen-woon-pc4.parquet",
      gem = "parquet/banen-woon-gem.parquet"
    ),
    fields = list(
      year = list(
